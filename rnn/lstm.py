...
cost = lasagne.objectives.squared_error(255*predictions, target_values).mean()
#ost = T.mean(1 - T.batched_dot(255*predictions,target_values)/(tnorm(255*predictions)*tnorm(target_values)))
# print cost
...
try:
    for epoch in range(NUM_EPOCHS):
        ...
        cost_val = compute_cost(X_train, Y_train)
        pred = probs(np.array([X_test[0]]))
        pred = np.array(255*pred,dtype = "uint8")

        # cv2.imshow("pred",(255*pred).reshape(64,64).astype("uint8"))
        # cv2.waitKey(200)
        # cv2.imshow("pred",X_test[1][0].reshape(64,64).astype("uint8"))
        # cv2.waitKey(200)
